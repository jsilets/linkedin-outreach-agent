...
const INTENT_RULES: Array<{ intent: ReplyIntent; test: (s: string) => boolean }> = [
  ...
  {
    intent: 'NotNow',
    test: (s) => /not now|later|next quarter|circle back|busy right now/.test(s),
  },
  {
    intent: 'Referral',
    test: (s) => /talk to|reach out to|my colleague|refer you|speak with/.test(s),
  },
  ...
  {
    intent: 'Interested',
    test: (s) => /interested|tell me more|sounds good|let's talk|keen/.test(s),
  },
];
