...
function connectReq(): ActRequest {
  return { type: 'connect', accountId: ACCT, targetId: TGT, campaignId: CAMP, payload: 'let us connect' };
}
...
describe('DispatchTick send-safety — approved send guards', () => {
  ...
  it('a lead reply (pullTargetFromFunnel) cancels the approved message; the tick does not send', async () => {
    ...
    const tick = new DispatchTick({ sequence: store.sequence, targets: store.target, messages: store.message, gate: makeGate(executor) });
  ...
  it('a progress already in replied cancels the approved message pre-send', async () => {
    ...
    const tick = new DispatchTick({ sequence: store.sequence, targets: store.target, messages: store.message, gate: makeGate(executor), log });
    const res = await tick.runTick(new Date());

    expect(executor.calls).toHaveLength(0);
    expect((await store.message.findById(msg.id))!.status).toBe('cancelled');
    expect(res.outcomes[0]).toMatchObject({ kind: 'cancelled', reason: 'replied', messageId: msg.id });
  ...
  it('an approved connect executes then parks awaiting_connection (does not advance past connect)', async () => {
    ...
    await store.sequence.upsertCampaignStep({ campaignId: CAMP, stepOrder: 0, stepType: 'connect', note: 'let us connect' });
    await store.sequence.upsertCampaignStep({ campaignId: CAMP, stepOrder: 1, stepType: 'message', body: 'follow' });
    const prog = await store.sequence.enrollTarget(CAMP, TGT, ACCT);
    await store.sequence.advanceTargetProgress(prog.id, { state: 'awaiting_approval' });
...
describe('DispatchTick send-safety — sequence step guards', () => {
  ...
  it('a message step reply probe that finds a reply holds and pulls the funnel', async () => {
    ...
    const probe = new FakeReplyProbe(store, 'reply');
    const tick = new DispatchTick({ sequence: store.sequence, targets: store.target, messages: store.message, gate: makeGate(executor), replyProbe: probe });
    const res = await tick.runTick(new Date());
  ...
  it('a message step reply probe that throws holds (reply_probe_failed) and leaves the cursor for retry', async () => {
    ...
    const tick = new DispatchTick({ sequence: store.sequence, targets: store.target, messages: store.message, gate: makeGate(executor), replyProbe: new FakeReplyProbe(store, 'throw') });
    const res = await tick.runTick(new Date());
